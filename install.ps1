...
<#
.SYNOPSIS
    Windows installer for hermes-switch-manager via Docker Desktop.
#>
param(
    [switch]$SkipStart
)
...
Write-Host "=== Installing hermes-switch-manager ===" -ForegroundColor Cyan
...
if (-not $SkipStart) {
    Write-Host "Building and starting hermes-switch-manager on port 3000..." -ForegroundColor Green
    Invoke-Expression "$composeCmd up --build -d"
    Write-Host "hermes-switch-manager installed. Access: http://localhost:3000" -ForegroundColor Green
} else {
